...
//   if (loading) {
//     return <p className="text-gray-600 text-center text-xl">Loading product details...</p>;
//   }

//   return (
//     <div className="p-10 bg-gradient-to-br from-blue-100 via-indigo-50 to-indigo-200 rounded-3xl shadow-2xl">
//       <div className="flex items-center justify-between mb-8">
//         <button
//           onClick={() => navigate(-1)}
//           className="px-6 py-3 bg-blue-500 text-white rounded-xl shadow-lg hover:bg-blue-600 transition-transform transform hover:scale-105"
//         >
//           ← Back
//         </button>
//         <h2 className="text-5xl font-extrabold text-blue-700">Product List</h2>
...
//           <table className="w-full table-auto border border-blue-300 rounded-xl">
...
//                   } hover:bg-indigo-100 transition-transform transform hover:scale-[1.01]`}
//                 >
//                   <td className="border p-4 text-center">{product.id || "N/A"}</td>
//                   <td className="border p-4 text-center font-medium">{product.name || "N/A"}</td>
//                   <td className="border p-4 text-center">₹{product.price || "N/A"}</td>
//                   <td className="border p-4 text-center">{product.stock || "N/A"}</td>
          ...
          <table className="w-full table-auto border border-blue-300 rounded-xl shadow-lg">
            ...
            <tbody>
              {products.map((product, index) => (
                ...
                >
                  <td className="border p-4 text-center font-bold text-blue-700">{product.id || "N/A"}</td>
                  <td className="border p-4 text-center font-medium text-gray-800">{product.name || "N/A"}</td>
                  <td className="border p-4 text-center text-green-600">₹{product.price || "N/A"}</td>
                  <td className="border p-4 text-center text-purple-600">{product.stock || "N/A"}</td>
                  <td className="border p-4 text-center">
                    <Link to={'/adminpage/editproducts'}>
                      <button
                        onClick={() => navigate(`/products/edit/${product.id}`)}
